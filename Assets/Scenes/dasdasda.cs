...
public class SwipeSimulator : MonoBehaviour
{
    public Vector2 startTouchPosition; // スワイプ開始位置
    public Vector2 endTouchPosition;   // スワイプ終了位置
    ...
    private IEnumerator SwipeRoutine()
    {
        while (true)
        {
            ...
            // スワイプの開始
            SimulateSwipe(startTouchPosition, endTouchPosition);
        }
    }
    ...
    private void SimulateTouch(Vector2 position, TouchPhase phase)
    {
        ...
        switch (phase)
        {
            case TouchPhase.Began:
                ExecuteEvents.Execute(EventSystem.current.currentSelectedGameObject, touchData, ExecuteEvents.pointerDownHandler);
                break;
            case TouchPhase.Moved:
                ExecuteEvents.Execute(EventSystem.current.currentSelectedGameObject, touchData, ExecuteEvents.dragHandler);
                break;
            case TouchPhase.Ended:
                ExecuteEvents.Execute(EventSystem.current.currentSelectedGameObject, touchData, ExecuteEvents.pointerUpHandler);
                break;
        }
    }
}
